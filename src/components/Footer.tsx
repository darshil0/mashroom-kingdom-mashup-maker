  ...
  return (
    <footer id="app-footer-decoration" className="fixed bottom-0 left-0 w-full z-40 bg-black/80 backdrop-blur-md border-t border-white/5 p-2 hidden sm:block">
       <div className="max-w-7xl mx-auto flex justify-between items-center opacity-40">
          <div id="footer-links" className="flex gap-4 items-center">
             <div className="flex gap-3 mr-4">
                {[
                  { label: 'LORE', id: 'link-lore', url: '/lore.html' },
                  { label: 'SPECS', id: 'link-specs', url: '/tech_specs.html' },
                  { label: 'CTRLS', id: 'link-controls', url: '/controls.html' },
                  { label: 'LOGS', id: 'link-logs', url: '/archive_v1.html' },
                  { label: 'API', id: 'link-api', url: '/api_reference.html' }
                ].map(link => (
                  <a 
                    id={link.id}
                    key={link.label} 
             ...
             <div id="input-telemetry" className="flex gap-1">
                {['W','A','S','D'].map(k => (
                  <span id={`key-hint-${k.toLowerCase()}`} key={k} className="px-1.5 py-0.5 border border-white/20 rounded text-[7px] font-black">{k}</span>
                ))}
             </div>
             <span id="txt-input-ready" className="text-[7px] font-black uppercase tracking-widest leading-none mt-0.5">Control_Input_Ready</span>
          </div>
