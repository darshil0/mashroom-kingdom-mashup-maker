...
app.post('/api/generate-level', async (req, res) => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `Generate a Mushroom Kingdom platformer level.
    ...
    // Ensure grid height constraint (15 tiles) and non-empty rows
    if (levelData.tiles.length !== 15) {
      console.warn(`Adjusting tiles array from height ${levelData.tiles.length} to 15`);
    }
