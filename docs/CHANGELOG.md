...
#### Fixed
- **Level Generation Validation**: Hardened the Gemini level-generation pipeline to reject malformed or incomplete payloads before they reach the game state.
- **Runtime Validation**: Added defensive validation to `src/services/geminiService.ts` to verify `LevelData` structure before accepting AI responses.
- **Serialization Safety**: Fixed tile/entity serialization to cover the full supported `TileType` set and reject invalid level payloads cleanly.
- **State Reset Stability**: Corrected player and entity reset behavior when the selected character or active level changes.
- **Win/Game Over Guards**: Prevented duplicate end-of-level callbacks from firing repeatedly within the same frame.
- **Dependency Hygiene**: Reconciled the package dependency tree and removed duplicate/conflicting package definitions across the Vite/React stack.

#### Added
- **Runtime configuration hardening**: The app now fails gracefully when the Gemini key is missing or the server receives an invalid AI response.
- **Improved server startup safety**: Added environment-aware defaults for host/port handling and proper Express/Vite boot flow.
- **Documentation alignment**: Updated setup guidance and project notes to reflect the current runtime, environment, and AI-generation flow.

#### Changed
- **Gemini model update**: Switched the generation model to a stable modern Gemini configuration better aligned with the project’s structured-output requirements.
- **Project setup flow**: Updated the runtime scripts and environment variables to better match the actual app behavior.
- **Version metadata**: Synced the package and documentation version markers to `1.8.2`.
